...
pendingTimers=[];}
function buildBoard(){boardEl.innerHTML="";reels=[];cells=[];for(var col=0;col<REEL_COUNT;col+=1){var reel=document.createElement("div");reel.className="jade-reel-b52a4f5d";var columnCells=[];for(var row=0;row<ROW_COUNT;row+=1){var cell=document.createElement("div");cell.className="jade-cell-b52a4f5d";var glyph=document.createElement("span");glyph.className="jade-glyph-b52a4f5d";glyph.textContent=drawSymbol().glyph;cell.appendChild(glyph);reel.appendChild(cell);columnCells.push(cell);}
...
function clearMarks(){for(var col=0;col<REEL_COUNT;col+=1){reels[col].classList.remove("jade-shimmer","jade-turning");for(var row=0;row<ROW_COUNT;row+=1){cells[col][row].classList.remove("jade-hit-b52a4f5d");}}
if(cabinetEl){cabinetEl.classList.remove("jade-blessed-b52a4f5d");}
var keys=Object.keys(payChips);for(var i=0;i<keys.length;i+=1){payChips[keys[i]].classList.remove("jade-hit-b52a4f5d");}}
function buildPayStrip(){payStripEl.innerHTML="";payChips={};for(var i=0;i<SYMBOLS.length;i+=1){var sym=SYMBOLS[i];var chip=document.createElement("li");chip.className="jade-pay-chip-b52a4f5d";chip.title="Three "+sym.label+" on the middle line";var glyphs=document.createElement("span");glyphs.className="jade-pay-glyphs-b52a4f5d";glyphs.textContent=sym.glyph+sym.glyph+sym.glyph;var pay=document.createElement("span");pay.className="jade-pay-x-b52a4f5d";pay.textContent="×"+sym.pay;chip.appendChild(glyphs);chip.appendChild(pay);payStripEl.appendChild(chip);payChips[sym.key]=chip;}}
...
tell("Two golden tigers hold the line — free respin!","");reels[oddCol].classList.add("jade-shimmer-b52a4f5d");setTurning(oddCol,true);later(function(){state.grid[oddCol]=drawColumn();setTurning(oddCol,false);reels[oddCol].classList.remove("jade-shimmer-b52a4f5d");showColumn(oddCol,state.grid[oddCol]);later(judgeRound,220);},RESPIN_MS);}
function settleWin(symbol){var prize=Math.round(state.stake*symbol.pay*100)/100;state.coins=Math.round((state.coins+prize)*100)/100;saveCoins();showCoins();for(var col=0;col<REEL_COUNT;col+=1){cells[col][CENTER_ROW].classList.add("jade-hit-b52a4f5d");}
if(payChips[symbol.key]){payChips[symbol.key].classList.add("jade-hit-b52a4f5d");}
if(cabinetEl){cabinetEl.classList.add("jade-blessed-b52a4f5d");}
